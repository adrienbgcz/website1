...
if ((empty($pseudo)) OR (empty($password)) OR (empty($password_again)) OR (empty($email)))
{
	...
	//Vérification du pseudo	
		...
		//Vérication du format de l'adresse mail
		...
		$req = $bdd->query('SELECT COUNT(*) AS existing_email FROM members WHERE email = \'' . $email . '\'');
		$donnees = $req->fetch();

		if ($donnees['existing_email'] > 0)
		{
...
if ((((preg_match("#^[a-zA-Z0-9\#!^$()[\]{}?+*.\|-]{8}$#", $password))) AND (($password) == ($password_again))) AND (preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $email)) AND ($donnees['existing_pseudo'] == 0) AND ($donnees['existing_email'] == 0)) 
{
	...
	echo '<p><a href="../pages/connection.php">ME CONNECTER</a></p>';
}
